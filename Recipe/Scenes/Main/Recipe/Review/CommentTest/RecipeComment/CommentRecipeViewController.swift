//
//  CommentRecipeViewController.swift
//  Recipe
//
...
class CommentRecipeViewController: BaseViewController {
    
    lazy var viewModel = CommentRecipeViewModel(recipeComment: comment)
        ...
        viewModel.likeForChildRelay.subscribe(onNext: { (id, index, section) in
            Task {
                ...
                if self.comment.data.content[section].replyList[index].is_likes {
                    LoginService.shared.sendRequest(url: NetworkEndpoint.recipeCommentUnLikeChild("\(id)").url,method: .get) { (data: DefaultReturnBool) in
                        if data.data {
...
//                }
                /// 좋아요를 누르지 않은 상태라면? -> 수락 하는 요청
                    ...
                    LoginService.shared.sendRequest(url: NetworkEndpoint.recipeCommentLikeChild("\(id)").url,method: .get) { (data: DefaultReturnBool) in
                        if data.data {
                            self.comment.data.content[section].replyList[index].reply_likes += 1
...
//                    }
        ...
        tableView.register(CommentRecipeCell.self, forCellReuseIdentifier: CommentRecipeCell.identifier)
        tableView.register(CustomFirstTableViewHeaderFooterViewForRecipe.self, forHeaderFooterViewReuseIdentifier: "CustomFirstTableViewHeaderFooterViewForRecipe")
            ...
            .subscribe(onNext: { _ in
                if let text = self.textView.text, text.contains("@"), let info = self.hashTagInfo {
                        ...
                        Task {
                            ...
                            if data {
                                Task {
                                    let data: RecipeComment = try await NetworkManager.shared.get(.recipeComment("\(self.divideId)"))
                                    self.comment = data
                                    self.viewModel.reloadData.accept(self.comment)
                                }
                                DispatchQueue.main.async {
                    ...
                    Task {
                        if let text = self.textView.text {
                            LoginService.shared.sendCommentForRecipe(comment: text, id: self.divideId) { data in
...
private extension CommentRecipeViewController {
    func keyboardCheck() {
