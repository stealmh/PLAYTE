...
class CommentRecipeViewModel : NSObject, UITableViewDataSource, UITableViewDelegate, CommentFooterViewDelegate {
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        if let headerView = tableView.dequeueReusableHeaderFooterView(withIdentifier: "CustomFirstTableViewHeaderFooterViewForRecipe") as? CustomFirstTableViewHeaderFooterViewForRecipe {
...
//            }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if let cell = tableView.dequeueReusableCell(withIdentifier: CommentRecipeCell.identifier, for: indexPath) as? CommentRecipeCell {
            cell.delegate = self
...
//MARK: - Header Action
extension CommentRecipeViewModel: CommentHeaderViewDelegate {
...
//MARK: - Cell Action
extension CommentRecipeViewModel: CommentRecipeCellDelegate {
...
//MARK: - 댓글용
extension CommentRecipeViewModel {
    /// 댓글을 등록할 때 필요합니다
    func registerComment(id: Int, parameter: [String: Any]) async -> Bool {
        do {
            let data1: DefaultReturnID = try await NetworkManager.shared.get(.createRecipeComment("\(id)"),parameters: parameter)
    ...
    func likeComment(id: Int) async -> Bool {
        do {
            let data1: DefaultReturnBool = try await NetworkManager.shared.get(.recipeCommentLike("\(id)"))
            if data1.code == "SUCCESS" {
                return true
            } else {
                return false
    ...
    func unlikeComment(id: Int) async -> Bool {
        do {
            let data1: DefaultReturnBool = try await NetworkManager.shared.get(.recipeCommentUnLike("\(id)"))
            if data1.code == "SUCCESS" {
                return true
            } else {
                return false
    ...
    func reportComment(id: Int) async -> Bool {
        do {
            let data1: DefaultReturnBool = try await NetworkManager.shared.get(.recipeCommentReport("\(id)"))
            if data1.code == "SUCCESS" {
                return true
            } else {
                return false
...
extension CommentRecipeViewModel {
    ...
    func registerCommentReply(id: Int, parameter: [String: Any]) async -> Bool {
        do {
            let data1: DefaultReturnID = try await NetworkManager.shared.get(.createRecipeCommentChild("\(id)"),parameters: parameter)
            if data1.code == "SUCCESS" {
                return true
            } else {
                return false
    ...
    func likeCommentReply(id: Int) async -> Bool {
        do {
            let data1: DefaultReturnBool = try await NetworkManager.shared.get(.recipeCommentLikeChild("\(id)"))
            if data1.code == "SUCCESS" {
                return true
            } else {
                return false
    ...
    func unlikeCommentReply(id: Int) async -> Bool {
        do {
            let data1: DefaultReturnBool = try await NetworkManager.shared.get(.recipeCommentUnLikeChild("\(id)"))
            if data1.code == "SUCCESS" {
                return true
            } else {
                return false
    ...
    func reportCommentReply(id: Int) async -> Bool {
        do {
            let data1: DefaultReturnBool = try await NetworkManager.shared.get(.recipeCommentReportChild("\(id)"))
            if data1.code == "SUCCESS" {
                return true
            } else {
                return false
